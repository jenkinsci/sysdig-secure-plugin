...
public class PolicyReportProcessor implements ReportProcessor {
  ...
  public PolicyEvaluationReport processPolicyEvaluation(ScanResult result) {
    ...
    logger.logDebug(String.format("sysdig-secure-engine policies for '%s': %s ", result.metadata().pullString(), evaluationPolicies.toString()));
      ...
      .flatMap(failedPolicy -> failedPolicy.bundles().stream()
        ...
        .flatMap(failedBundle -> failedBundle.rules().stream()
          ...
          .flatMap(failedRule -> failedRule.failures().stream()
            .map(failure ->
              new PolicyEvaluationReportLine(scanResult.metadata().imageID(),
  ...
  @Override
  public PolicyEvaluationSummary generateGatesSummary(@NonNull PolicyEvaluationReport gatesJson, @NonNull ScanResult imageScanningResult) {
    ...
    for (var imageKey : gatesJson.getResultsForEachImage().entrySet()) {
      ...
      logger.logInfo(String.format("Policy evaluation summary for %s - stop: %d (+%d whitelisted), warn: %d (+%d whitelisted), go: %d (+%d whitelisted), final: %s", imageScanningResult.metadata().pullString(), stop - stop_wl, stop_wl, warn - warn_wl, warn_wl, go - go_wl, go_wl, finalAction));

      gateSummary.addSummaryLine(imageScanningResult.metadata().pullString(), (stop - stop_wl), (warn - warn_wl), (go - go_wl), finalAction);
    }
