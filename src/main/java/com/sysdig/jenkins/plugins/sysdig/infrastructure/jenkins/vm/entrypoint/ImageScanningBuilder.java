...
public class ImageScanningBuilder extends Builder implements SimpleBuildStep {
    ...
    public void perform(
        ...
        try (reportStorage) {
            ImageScanningApplicationService scanningApplicationService =
                    new ImageScanningApplicationService(reportStorage, scanner, logger);
            logger.logWarn(String.format(
                    "Starting Sysdig Secure Container Image Scanner step, project: %s, job: %d",
                    runContext.getProjectName(), runContext.getJobNumber()));
            scanningApplicationService.runScan(config);
            logger.logWarn(String.format(
                    "Completed Sysdig Secure Container Image Scanner step, project: %s, job: %d",
                    runContext.getProjectName(), runContext.getJobNumber()));
        }
    }
    ...
    public static final class GlobalConfiguration extends BuildStepDescriptor<Builder> {
        ...
        public FormValidation doCheckImageName(@QueryParameter String value) {
            return Strings.isNullOrEmpty(value)
                    ? FormValidation.error("Please enter a valid image name")
                    : FormValidation.ok();
        }
